//functions
function isElementBelowBottom(e,documentHeight){
    let rect = e.getBoundingClientRect();
    return rect.bottom > documentHeight
}
...
function addEmojisButton(commBox){
	...
	if ((commBox)&&(commBox.matches("form"))&&(commBox.querySelector(`.${class_EmojisButton}`)==null)){
		...
		if (isProfilePage){uiBar.classList.add("Scratch2ProfileUiBar")}
		...
		EmojisIcon.src = getFileUrl("images/icons/taco.svg")
...
function addEmojisSelector(uiBar){
	...
	if(!(uiBar.querySelector(".EmojisSelectorContainer"))){
		...
		let oldDocumentHeight = document.documentElement.scrollHeight
		...
		//selector settings
		if(darkMode){EmojisSelector.classList.add("darkTheme")}
		if(isElementBelowBottom(EmojisSelector,oldDocumentHeight)){EmojisSelector.classList.add("reachedEdge")}
...
function setEmojisSelectorContent(EmojisSelector){
	//append search bar
	let SearchBar = document.createElement("input");SearchBar.classList.add("SearchEmojis");if(darkMode){SearchBar.classList.add("darkTheme")};SearchBar.placeholder = "🔎︎ Search..."
	...
	for (let value of EmojisData){
		...
		let emoji = document.createElement("div");emoji.classList.add("ScratchEmojisEmoji","unselectable");if(darkMode){emoji.classList.add("darkTheme")}
...
var darkMode = !(getComputedStyle(document.body).getPropertyValue('--darkWww-page')=="")

var css_RootCommContainer, css_CommBubbles, css_CommUiBar
var css_textArea, css_WordsLeftText

var class_EmojisButton = "EmojisButton"

if (isProfilePage){
	//profile page css selectors
	...
	//projects and studio css selectors
...
const BodyObserver = new MutationObserver(scanAllAndAddObservers);BodyObserver.observe(document.body,{subtree:true,childList:true})